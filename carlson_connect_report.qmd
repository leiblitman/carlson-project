...
# 10 Change Score Analysis

This section examines *who* changes and *how much* in response to the AI conversation.
Four questions are addressed:

1. **What do the change score distributions look like?** (histograms)
2. **Do the baseline attitude clusters differ in how much they change?**
3. **Which demographic characteristics predict change?**
4. **Are there distinct change-pattern clusters?** (independent k-means on Δ scores)

```{r}
#| label: change-prep

# ── Define all 20 pre/post column pairs ──────────────────────────────────────
ch_pre  <- c(cred_pre_cols, fp_pre_cols, therm_pre_cols, tc_pre_cols, ip_pre_cols)
ch_post <- c(cred_post_cols, fp_post_cols, therm_post_cols, tc_post_cols, ip_post_cols)
...
# ── Compute Δ = post − pre per row ───────────────────────────────────────────
delta_mat <- map2_dfc(ch_pre, ch_post, function(p, q) {
  suppressWarnings(as.numeric(df_c[[q]])) - suppressWarnings(as.numeric(df_c[[p]]))
}) |> setNames(ch_labels)

# ── Attach baseline cluster + demographics ───────────────────────────────────
# km_final ran on df (all rows); we need only the completed-respondent rows.
...
delta_full <- delta_mat |>
  mutate(
    ...
    party3   = analysis_df$party3,
    ideology = analysis_df$ideology,
    ideo_num = analysis_df$ideo_num,
    age_num  = analysis_df$age_num,
    race_grp = analysis_df$race_grp
  )
...
## 10.1 Distributions of Change Scores

```{r}
#| label: fig-change-hist
#| fig-height: 10
#| fig-cap: "Distribution of pre-to-post change scores (Δ = post − pre) for all 20 outcome items"

delta_summary <- delta_long |>
  group_by(domain, item) |>
  summarise(M = mean(delta), .groups = "drop")

delta_long |>
  ggplot(aes(x = delta)) +
  geom_histogram(binwidth = 10, fill = "#2c7bb6", color = "white", alpha = 0.8) +
  geom_vline(data = delta_summary, aes(xintercept = M),
             color = "#d7191c", linetype = "dashed", size = 0.8) +
  geom_vline(xintercept = 0, color = "grey40", size = 0.5) +
  scale_x_continuous(breaks = c(-50, 0, 50)) +
  facet_wrap(~item, ncol = 4, scales = "free_y") +
  labs(
    title    = "Pre → Post Change Score Distributions",
...
## 10.2 Baseline Clusters and Change

Do participants in different *baseline* attitude clusters (Section 7) respond differently to the AI conversation?

```{r}
#| label: fig-cluster-change
#| fig-height: 8
#| fig-cap: "Mean change score by baseline attitude cluster and outcome item"

cluster_change <- delta_full |>
  pivot_longer(all_of(ch_labels), names_to = "item", values_to = "delta") |>
  filter(!is.na(delta)) |>
...
ggplot(cluster_change, aes(x = M, y = item, color = baseline_cluster,
                            shape = baseline_cluster)) +
  geom_vline(xintercept = 0, linetype = "dashed", color = "grey60") +
  geom_errorbarh(aes(xmin = M - 1.96*SE, xmax = M + 1.96*SE),
                 height = 0.3, position = position_dodge(0.6)) +
  geom_point(size = 2.8, position = position_dodge(0.6)) +
  scale_color_manual(values = pal_main[1:best_k], name = "Baseline cluster") +
  scale_shape_manual(values = c(16,17,15,18)[1:best_k], name = "Baseline cluster") +
  facet_grid(domain ~ ., scales = "free_y", space = "free_y") +
  labs(
    title    = "Mean Change by Baseline Attitude Cluster",
...
```{r}
#| label: tbl-cluster-change-anova
#| tbl-cap: "Does baseline cluster predict magnitude of change? One-way ANOVA per item."

cluster_anova <- map_dfr(ch_labels, function(item) {
  d <- delta_full |> filter(!is.na(.data[[item]]), !is.na(baseline_cluster))
  if (n_distinct(d$baseline_cluster) < 2 || nrow(d) < 10) return(NULL)
  aov_res <- aov(reformulate("baseline_cluster", response = item), data = d)
  sm      <- summary(aov_res)[[1]]
  tibble(
    Item   = item,
    Domain = ch_domains[match(item, ch_labels)],
...
cluster_anova |>
  mutate(p = paste0(format(round(p, 3), nsmall = 3), " ", sig)) |>
  select(-sig) |>
  kbl(caption = "ANOVA: Baseline cluster × change score") |>
  kable_styling(bootstrap_options = c("striped","hover","condensed"),
                full_width = FALSE, position = "left") |>
...
## 10.3 Demographic Predictors of Change

OLS regressions with Δ (change score) as the outcome and party, ideology, age, and race as predictors.

```{r}
#| label: tbl-change-regression
#| tbl-cap: "OLS regression: demographic predictors of change scores (ref: Republican, Very conservative, White)"

reg_delta_df <- delta_full |>
  mutate(
    party3   = factor(party3,   levels = c("Republican","Ind./Other","Democrat")),
    ideology = factor(ideology, levels = c("Very conservative","Somewhat conservative",
                                           "Moderate","Somewhat liberal","Very liberal")),
    race_grp = factor(race_grp, levels = c("White","Non-White"))
  )

delta_reg_results <- map_dfr(ch_labels, function(item) {
  d   <- reg_delta_df |> filter(!is.na(.data[[item]]))
  fml <- reformulate(c("party3","ideology","age_num","race_grp"), response = item)
  m   <- lm(fml, data = d)
  sm  <- summary(m)
  est  <- coef(sm)[,"Estimate"]
  pval <- coef(sm)[,"Pr(>|t|)"]

  get <- function(nm) {
    v <- est[nm]; p <- pval[nm]
    if (is.na(v)) return("—")
    paste0(round(v, 1), " ", sig_star(p))
  }
  tibble(
    Item        = item,
    Domain      = ch_domains[match(item, ch_labels)],
    `Ind./Other`= get("party3Ind./Other"),
    Democrat    = get("party3Democrat"),
    Moderate    = get("ideologyModerate"),
    `Smwt.lib.` = get("ideologySomewhat liberal"),
    Age         = get("age_num"),
    `Non-White` = get("race_grpNon-White"),
    R2          = round(sm$r.squared, 2)
  )
})

delta_reg_results |>
  kbl(caption = "OLS Δ Regressions (ref: Republican, Very conservative, White)") |>
  kable_styling(bootstrap_options = c("striped","hover","condensed"),
                full_width = TRUE) |>
  add_header_above(c(" "=1," "=1,
                     "Party (ref=Rep.)"=2,
                     "Ideology (ref=Very con.)"=2,
                     " "=1," "=1," "=1)) |>
  collapse_rows(columns = 2, valign = "top") |>
  footnote(general = "Coefficients = unstandardized B. † p<.10, * p<.05, ** p<.01, *** p<.001.",
           general_title = "")
```

## 10.4 Cluster Analysis on Change Scores

Independent k-means clustering on the 20 change score variables identifies **distinct change-pattern profiles** — groups of participants who changed in similar ways (or didn't change) across the outcome battery.

```{r}
#| label: change-cluster-prep

# Build a complete-case change matrix for clustering
delta_clust_mat <- delta_mat |>
  mutate(across(everything(),
                ~ifelse(is.na(.x), 0, .x))) |>   # impute no-change for missing
  as.matrix()

# Scale by SD so no single high-variance item dominates
delta_scaled <- scale(delta_clust_mat)
delta_scaled[!is.finite(delta_scaled)] <- 0       # guard against zero-variance cols
...
#| fig-height: 3.5
#| fig-cap: "Silhouette scores for change-score clustering"
...
```{r}
#| label: fig-change-cluster-pca
#| fig-height: 5
#| fig-cap: "Change-score clusters projected onto first two principal components"
...
pca_ddf |>
  ...
  scale_fill_manual(values  = pal_main[1:best_k_delta], name = "Change cluster") +
  ...
  labs(
    ...
    subtitle = paste0("PC1 = ", var_exp_d[1], "% variance; PC2 = ", var_exp_d[2], "%"),
...
```{r}
#| label: tbl-change-cluster-profile
#| tbl-cap: "Mean change score (Δ) by change cluster"

delta_profile_tbl <- as_tibble(delta_clust_mat) |>
  ...
  mutate(Domain = ch_domains[match(Item, ch_labels)],
         col_label = paste0("Cluster ", change_cluster, " (n=", n, ")")) |>
...
```{r}
#| label: fig-change-cluster-profile
#| fig-height: 8
#| fig-cap: "Change cluster mean profiles across all 20 outcome items"
...
```{r}
#| label: tbl-cluster-crosstab
#| tbl-cap: "Cross-tabulation: baseline attitude clusters × change-score clusters"
...
with(
  ...
  kbl(caption = "Cross-tab: Baseline Attitude Clusters × Change-Score Clusters") |>
  kable_styling(bootstrap_options = c("striped","hover","condensed"),
                full_width = FALSE, position = "left") |>
  footnote(general = "Cell values = n. Check for non-random associations between baseline position and change pattern.",
           general_title = "")
```

```{r}
#| label: tbl-change-cluster-demo
#| tbl-cap: "Demographic composition of change-score clusters"
...
tibble(
  ...
  summarise(
    ...
    `% Very/Smwt. Conservative` = paste0(
      round(mean(ideology %in% c("Very conservative","Somewhat conservative"),
                 na.rm=TRUE)*100, 0), "%"),
    `Mean Age`    = round(mean(age_num, na.rm=TRUE), 1)
  ) |>
  rename(`Change Cluster` = change_cluster) |>
  kbl(caption = "Demographic Profile of Change Clusters") |>
  kable_styling(bootstrap_options = c("striped","hover","condensed"),
                full_width = FALSE, position = "left")
```
